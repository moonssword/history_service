...
export async function logAction(request: FastifyRequest<{ Body: ActionRequestBody }>, reply: FastifyReply) {
...
export async function getActions(request: FastifyRequest, reply: FastifyReply) {
    const { shop_id, plu, date_from, date_to, action, page = 1, limit = 10 } = request.query as {
        shop_id?: number;
        plu?: string;
        date_from?: string;
        date_to?: string;
